...
+ (void)hiddenStatusBarAnimated:(BOOL)animated
{
    if (animated) {
        ...
        [UIView animateWithDuration:kStatusBarAnimationDuration animations:^{
            [MSStatusBarWindow sharedManager].alpha = 0.f;
        } completion:^(BOOL finished) {
            if (finished) {
                [MSStatusBarWindow sharedManager].hidden = YES;
                [[MSStatusBarWindow sharedManager].array removeAllObjects];
                for (UIView *view in [MSStatusBarWindow sharedManager].subviews) {
                    [view removeFromSuperview];
                }
                [MSStatusBarWindow sharedManager].statusBar0 = nil;
                [MSStatusBarWindow sharedManager].statusBar1 = nil;
                [MSStatusBarWindow sharedManager].isAnimating = NO;
            }
        }];
    }
    else{
            ...
            [UIView animateWithDuration:kStatusBarAnimationDuration
                             ...
                             animations:^{
                                 ...
                                 if (finished) {
                                     // 出列
                                     if([[MSStatusBarWindow sharedManager].array count] > 0) {
                                         [[MSStatusBarWindow sharedManager].array removeObjectAtIndex:0];
                                     }
...
- (void)playMultiEndAnimation
{
    ...
    [UIView animateWithDuration:kStatusBarAnimationDuration
        ...
        if (finished) {
            if (!self.isAutoHidden && [[MSStatusBarWindow sharedManager].array count] == 1) {
                // 不是自动隐藏 且只有自己一个
                // 那就一直显示着
            }
            else{
